...
resistor in the chain and the total resistance, RT, of the circuit must be equal to the sum of all the individual resistors added together. That is:
RT = R1 + R2 + R3 ...
...
function sumOfResistanceInSeriesCircuit(Resistance){
    var sum_of_resistance=0;
    if(Resistance<=1)
    {
        return 1;
    }
        ...
        for (let i = 0; i < Resistance.length; i++) {
            sum_of_resistance += Resistance[i];
        }
    }
    return sum_of_resistance;
}
console.log(sumOfResistanceInSeriesCircuit([0.5,0.5],"ohms"));